...
// Zakładam, że NavLinkItem oraz MobileNavbarWrapper
// są Twoimi komponentami (client components).
import { NavLinkItem, MobileNavbarWrapper } from "@/components/navigation";

// Upewnij się, że ten komponent jest "server component" (domyślnie w App Router tak jest)
// Jeśli byłby on w folderze /components/ to możesz potrzebować dopisać:
// `export const dynamic = "force-static";` lub inne ustawienia, w zależności od potrzeb.

export default async function TopNavigation() {
  // Definiujemy zapytanie GraphQL do Strapi:
  const query = `
    query Navigation {
      navigation {
        PageName
  ...
  const response = await fetch("http://localhost:1337/graphql", {
    ...
    // Domyślnie w App Router fetch ma caching: 'force-cache'.
    // Jeżeli potrzebujesz ISR (Incremental Static Regeneration), ustaw np.:
    next: { revalidate: 60 }, // rewalidacja co 60s
  });

  // Parsujemy odpowiedź (JSON) i wyciągamy dane
  const { data } = await response.json();
  const navigationData = data?.navigation;

  // Jeśli Strapi zwróciło pustą nawigację, można dodać fallback:
  if (!navigationData) {
    return (
      <header className="flex items-center justify-between gap-10 py-4 container">
        <div>Brak danych nawigacyjnych</div>
      </header>
    );
  }

  // Wyciągamy linki i CTA
  ...
  return (
    <header className="flex items-center justify-between gap-10 py-4 container">
      <Link href="/" className="flex items-center gap-3">
        ...
        <span className="font-pixelify text-xl md:text-2xl font-bold">
          {/* Możesz wyrenderować np. PageName jeśli chcesz */}
          {navigationData.PageName ?? "Rafal Blog"}
        </span>
      </Link>

      <div className="flex items-center gap-10">
        {/* Duża nawigacja (widoczna na desktopie) */}
        <nav className="hidden items-center gap-10 md:flex justify-end">
          {Links?.map((navItem: any) => (
            <NavLinkItem
              href={navItem.href}
              // Załóżmy, że w Strapi nie przechowujesz isExternal
              // - możesz zawsze dodać warunek lub dedykowane pole
              isExternal={false}
              key={navItem.id}
            >
              {navItem.label}
